...
if [ -n "$SPECIFIC_RESOURCES" ]; then
    echo "Destroying specific resources: $SPECIFIC_RESOURCES"
    ...
    for resource_id in $SPECIFIC_RESOURCES; do
        ...
        case $resource_type in
            "s3")
                echo "Destroying S3 bucket: $resource_name"
                $AWS_CMD s3 rb s3://$resource_name --force 2>/dev/null || true
                $AWS_CMD s3api delete-bucket --bucket $resource_name 2>/dev/null || true
                echo "Deleted S3 bucket: $resource_name"
                ;;
            "dynamodb")
                echo "Destroying DynamoDB table: $resource_name"
                $AWS_CMD dynamodb delete-table --table-name $resource_name 2>/dev/null || true
                echo "Deleted DynamoDB table: $resource_name"
                ;;
            "lambda")
                echo "Destroying Lambda function: $resource_name"
                $AWS_CMD lambda delete-function --function-name $resource_name 2>/dev/null || true
                echo "Deleted Lambda function: $resource_name"
                ;;
            "apigateway")
                echo "Destroying API Gateway: $resource_name"
                $AWS_CMD apigateway delete-rest-api --rest-api-id $resource_name 2>/dev/null || true
                echo "Deleted API Gateway: $resource_name"
                ;;
            *)
                echo "Unknown resource type: $resource_type for resource: $resource_id"
                ;;
        esac
    done
else
    echo "No specific resources provided, destroying all resources for project: $PROJECT_NAME"
...
echo "Resource destruction completed for project: $PROJECT_NAME" 