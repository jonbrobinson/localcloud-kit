...
2. **Complete first-time setup:**
   ...
   - Generate SSL certificates for `localcloudkit.local`
...
- **Main Application**: `https://localcloudkit.local`
- **API Endpoints**: `https://localcloudkit.local/api/*`
- **WebSocket (Socket.IO)**: `wss://localcloudkit.local/ws/socket.io`
- **Health Check**: `https://localcloudkit.local/health`
- **LocalStack Health**: `https://localcloudkit.local/localstack/health`
...
- TLS/SSL termination
- Domain-based routing (`localcloudkit.local`)
...
- **Wildcard**: `*.localcloudkit.local` (for subdomains)
- **Validity**: 825 days
...
5. **Domain Not Resolving**
   ...
   # Or manually
   echo "127.0.0.1 localcloudkit.local" | sudo tee -a /etc/hosts
   ```
...
# Check LocalStack
curl -k https://localcloudkit.local/localstack/health
